...
module Global =
    ...
    | TypeTags.SuffixString ->
      ...
      let compiled = 
        Utils.trapSyntaxError env (fun () ->
          ...
          env.EvalCache.Lookup(source, (fun (_) ->
              let ast =
                target.Function.Env 
                |> Parser.parse source
                |> fst
              ...
              let _ = Ast.Function(None, scope, ast)
              let _ = Some(target.GlobalLevel, target.ClosureLevel)
              let env = target.Function.Env
  ...
  let private parseWithRadix (z:string, r:int) =
    ...
    if r = 2 && z.Length <= 64 then Convert.ToUInt64(z, 2) |> bigint.op_Implicit
      ...
      let r = new bigint(r)
      let accumulateDigit (value:bigint) (char:char) = 
        (value * r) + (new bigint(digits.IndexOf(char)))
  ...
  let parseInt (str:BV) (radix:BV) =
    ...
    if R <> 0 && (R < 2 || R > 36) then 
      ...
      if stripPrefix && S.Length >= 2 && (S.StartsWith("0x") || S.StartsWith("0X")) then
        ...
        S <- S.Substring(2)
      let Z =
        Regex.Match(S.ToUpperInvariant(), "^[" +
                                          "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ".Substring(0, R) +
                                          "]*").Value
  ...
  let parseFloat (str:BoxedValue) =
    ...
    let prefixMatch = 
      Regex.Match(trimmedString,
                  @"^[-+]?(Infinity|([0-9]+\.[0-9]*|[0-9]*\.[0-9]+|[0-9]+)([eE][-+]?[0-9]+)?)")
  ...
  let private decodeUTF8 (b:byte[]) : int =
    ...
    if count = 2 then
      ...
      if octetA &&& 0xE0 <> 0xC0 ||
         octetB &&& 0xC0 <> 0x80
        then raise (new UriFormatException("An attempt was made to decode an invalid character."))
      ...
      if octetA &&& 0xF8 <> 0xF0 ||
        ...
        then raise (new UriFormatException("An attempt was made to decode an invalid character."))
      ((((((octetA &&& 0x07) <<< 6) |||
          (octetB &&& 0x3F)) <<< 6) ||| (octetC &&& 0x3F)) <<< 6) ||| (octetD &&& 0x3F)
    else
      ...
      if C <> '%' then
        ...
        then raise (
              new UriFormatException(
                String.Format(
                  "The uri containted an invalid escape sequence ('{0}').",
                  s.Substring(start, 3))))
        k <- k + 2
        if B &&& 0x80uy = 0uy then
          ...
          then raise (
                new UriFormatException(
                  String.Format(
                    "The uri containted an invalid escape sequence ('{0}').", s.Substring(start))))
          ...
          while j < n do
            ...
            then raise (
                  new UriFormatException(
                    String.Format(
                      "The uri containted an invalid escape sequence ('{0}').",
                      s.Substring(start, k - start))))
            let isHex, B = parseByte s (k + 1)
            if not isHex
            then raise (
                  new UriFormatException(
                    String.Format(
                      "The uri containted an invalid escape sequence ('{0}').", s.Substring(k, 3))))
            if B &&& 0xC0uy <> 0x80uy
            then raise (
                  new UriFormatException(
                    String.Format(
                      "The uri containted an invalid escape sequence ('{0}').",
                      s.Substring(start, k - start + 2))))
            k <- k + 2
            Octets.[j] <- B
            j <- j + 1
          let V =
            try
              decodeUTF8 Octets
            with
              | :? FormatException as e -> raise (
                                            new UriFormatException(
                                              String.Format(
                                                "The uri containted an invalid escape sequence" +
                                                " ('{0}'). {1}",
                                                s.Substring(start, k - start), e.Message)))
          if V < 0x10000 then
            let C = char V
            if reservedSet.IndexOf(C) = -1 then
              R.Append(C) |> ignore
  ...
  let private encodeUTF8 (v:int) : byte[] =
    if v < 0x0080 then
      ...
      raise (
        new UriFormatException(
          String.Format("An attempt was made to encode an unknown character (0x{0:X}).", v)))
  ...
  let private encode (s:string) (unescapedSet:string) : string =
    ...
    while k <> strLen do
      ...
      if unescapedSet.IndexOf(C) <> -1 then
        ...
        then raise (
              new UriFormatException(
                String.Format("An invalid character was detected at position {0} (0x{1:X4}).",
                              k, cChar)))
        let V =
          if cChar < 0xD800 || cChar > 0xDBFF then
            ...
            k <- k + 1
            if k = strLen
            then raise (new UriFormatException("An incomplete URI string was detected."))
            let kChar = int s.[k]
            if kChar < 0xDC00 || kChar > 0xDFFF
            then raise (
                  new UriFormatException(
                    String.Format(
                      "An invalid character was detected at position {0} (0x{1:X4}).", k, kChar)))
