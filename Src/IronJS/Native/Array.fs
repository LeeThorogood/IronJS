  ...
  module Prototype = 
    ...
    let private toLocaleString (func:FO) (this:CO) =
      ...
      while i < length do
        match this.Get(i) with
        | x when x.IsUndefined || x.IsNull -> buffer.Append("") |> ignore
        | x -> buffer.Append(TC.ToObject(func.Env, x).CallMember("toLocaleString")|>TC.ToString) |>
               ignore
        buffer.Append(separator) |> ignore
        i <- i + 1u
    ...
    let splice (func:FO) (this:CO) (args:Args) =
      ...
      let actualStart =
        if relativeStart < 0 
        then Math.Max((len + relativeStart), 0) 
        else Math.Min(relativeStart, len)
