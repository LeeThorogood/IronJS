...
module internal String =
  ...
  module Prototype = 
    ...
    let private evaluateReplacement
      (matched:string) (before:unit->string) (after:unit->string) (replacement:string)
    ...
    let private replace (_:FO) (this:CO) (search:BV) (replace:BV) =
      ...
      if search.IsRegExp then 
        ...
        //replace(regex, string)
          ...
          let matchEval (m:Match) =
            ...
            let after =
              memoize (fun () -> value.Substring(Math.Min(value.Length - 1, m.Index + m.Length)))
      ...
      //replace(string, _)
        ...
        if index > -1 then
          ...
          if replace.IsFunction then 
            let replace =
              replace.Func.Call(this.Env.Globals, search, double index, value) |> TC.ToString
          ...
          //replace(string, string)
            ...
            let replace = replace |> TC.ToString
            let replace =
              evaluateReplacement search (fun () -> before) (fun () -> after) replace null
    ...
    let setup (env:Env) =
      ...
      let charCodeAt =
        FunctionReturn<double, double>(charCodeAt)  $ Utils.createFunction env (Some 1)
      ...
      let indexOf =
        FunctionReturn<string, double, double>(indexOf) $ Utils.createFunction env (Some 1)
      proto.Put("indexOf", indexOf, DescriptorAttrs.DontEnum)

      let lastIndexOf =
        FunctionReturn<BV, BV, double>(lastIndexOf) $ Utils.createFunction env (Some 1)
      proto.Put("lastIndexOf", lastIndexOf, DescriptorAttrs.DontEnum)

      let localeCompare =
        FunctionReturn<string, double>(localeCompare) $ Utils.createFunction env (Some 1)
      ...
      let substring =
        FunctionReturn<double, BV, string>(substring) $ Utils.createFunction env (Some 2)
      ...
      let toLocaleLowerCase =
        FunctionReturn<string>(toLocaleLowerCase) $ Utils.createFunction env (Some 0)
      ...
      let toLocaleUpperCase =
        FunctionReturn<string>(toLocaleUpperCase) $ Utils.createFunction env (Some 0)
