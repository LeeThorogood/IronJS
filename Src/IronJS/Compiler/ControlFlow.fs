...
module internal ControlFlow =
  ...
  let for' (ctx:Ctx) label init test incr body =
    ...
    let test = ctx $ compile test $ DlrTC.ToBoolean
  ...
  let forIn (ctx:Ctx) label target object' body =
    ...
    let pair = Dlr.paramT<System.Tuple<uint32, MutableSet<string>>> "pair"
    ...
    let propertyEnumerator =
      Dlr.paramT<System.Collections.Generic.IEnumerator<string>> "propertyEnumerator"
    #else
              ...
              (Dlr.if' propertyState
                (Dlr.blockSimple [
                  (Dlr.assign propertyState
                    ...
                    #if NET2
                    (Dlr.call (Dlr.castT<System.Collections.IEnumerator> propertyEnumerator)
                    "MoveNext" []))
                    #else
