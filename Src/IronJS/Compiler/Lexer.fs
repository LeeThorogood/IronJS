...
module internal Lexer =
  ...
  *)
  ...
  let private identifier (s:Input.T) (_:char) =
    s |> storePosition
    s |> bufferClear
  ...
  *)
  let private singlelineComment (s:T) =
    s |> bufferClear
    s |> advance
  ...
  *)
  let private multilineComment (s:T) =
    s |> storePosition
    s |> bufferClear
    s |> advance
  ...
  *)
  let private numericLiteral (c:char) (s:T) =
    s |> storePosition
    s |> bufferClear
    c |> buffer s
    s |> advance
    ...
    if s |> continue' then
      ...
      while s |> continue' &&
            (c |> isDecimal || c = 'e' || c = 'E' || (passedExponent && (c = '-' || c = '+')) ||
             (c = '.' && not passedDot)) do
  ...
  *)
  let private punctuation (s:T) (first:char) =
    s |> storePosition

    let inline makeToken (s:T) (buffer:string) =
      s |> outputSymbol punctuationMap.[buffer]

    //TODO: Unroll this recursive call manually
    let rec punctuation (s:T) (buffer:string) =
      s |> advance
  ...
  // such as 'foo' or 'bar'
  *)
  let private stringLiteral (s:T) (stop:char) =
    s |> storePosition
    s |> bufferClear

    let rec stringLiteral (s:T) =
      s |> advance
