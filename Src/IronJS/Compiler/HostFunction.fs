...
module internal HostFunction =
  ...
  let makeInvoke<'a when 'a :> Delegate and 'a : not struct> func arguments =
    ...
    if invoke.Type = typeof<Void> 
      then Dlr.Fast.block [||] [|invoke; Constants.Boxed.undefined|]
  ...
  let defaultArg env (type':Type) =
    if type' == typeof<BV> 
      then Constants.Boxed.undefined
      elif type' == typeof<CO> || type'.IsSubclassOf(typeof<CO>)
        then Dlr.default' type'
        else DlrTC.ConvertTo(env, Constants.Boxed.undefined, type')
      ...
      // Case 2: Host is variadic but callsite is static
        ...
        let hostArguments = 
          ...
          let hostVariadicArgs = 
            callsiteArgs 
            $ Array.map box     // Map over the callsite args and box all values
